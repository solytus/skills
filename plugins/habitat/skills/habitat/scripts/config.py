"""Habitat instance config (Tier-2): resolve the data root + tenant id and
...
DEFAULT_CONFIG_PATH = Path.home() / ".solytus" / "habitat" / "config.yaml"
DEFAULT_DATA_ROOT = Path.home() / ".solytus" / "habitat" / "data"
...
def resolve_data_root(config_path=None, env=None):
    """data root: HABITAT_DATA_ROOT env -> config `data_root` -> default."""
    env = os.environ if env is None else env
    if env.get("HABITAT_DATA_ROOT"):
        return Path(env["HABITAT_DATA_ROOT"]).expanduser()
...
def resolve_tenant_id(config_path=None, env=None):
    """tenant id: HABITAT_TENANT_ID env -> config `tenant_id` -> 'local'.
    ...
    env = os.environ if env is None else env
    if env.get("HABITAT_TENANT_ID"):
        return env["HABITAT_TENANT_ID"]
    cfg = load_instance_config(config_path)
...
def _main(argv=None):
    ...
    ap = argparse.ArgumentParser(
        description="Resolve Habitat instance config (data root, tenant, version)."
    )
